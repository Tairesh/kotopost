    ...
    uploadHttpRequest.onload = function () {
        ...
        documentUploadServer.onload = function () {
            ...
            if (answer.response.upload_url === undefined) {
                thereIsAnError('Ошибка', answer, imageUrl);
...
var caption = '%23cats%23cat%23kitten%23kittens%23коты%23кот%23котята%23котенок%23котэ%23кошка%23котики%23котаны%23котик%23котейка%23ня%23няша%23няшка';
/*for (var i = 0; i < 30; i++) {
...
//console.log(url);
                ...
                documentSaveRequest.onload = function () {
                    ...
                    var answer = JSON.parse(documentSaveRequest.response);
//console.log(answer);
                    if (answer.response === undefined || answer.response.post_id === undefined) {
                        thereIsAnError('Ошибка', answer, imageUrl);

                        return;
                    }

                    document.getElementById('wrap').innerHTML = '<p></p><br/><br/><center><h1>Картинка отправлена!</h1></center><br/>';
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (params === undefined || params.length ===  undefined || params.length < 2) {
        thereIsAnError('Ошибка', 'params || params.length != 2', imageUrl);
        return;
    }
    ...
    if (filename.length === undefined || filename.length === 0) {
        thereIsAnError('Ошибка', 'filename.length <= 0', imageUrl);
        return;
    }
